...
function CustomerDetail() {
  ...
  const { data: utilityBills, isPending: isUtilityBillsPending } = useGetUtilityBillsByCustomer(id);
                      ...
                      <div key={agreement.id} className={`p-3 rounded-md ${statusStyle.container}`}>
                        ...
                        {agreement.status === "active" && (
                          <div className="flex gap-2 mt-2">
                            <button
                              onClick={() => navigate(`/admin/payments/create/${agreement.id}`)}
                              className="px-3 py-1 text-sm bg-blue-600 text-white rounded-md hover:bg-blue-700"
                            >
                              Add Payment
                            </button>
                            <button
                              onClick={() => navigate(`/admin/utility-bills/create/${agreement.id}`)}
                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          {bill.paid_date && (
                            <p className="text-sm text-gray-600">Paid Amount: Rs. {bill.paid_amount}</p>
                          )}
                          ...
                          {bill.paid_date && (
                            <p className="text-sm text-gray-600">Paid Date: {formatDate(bill.paid_date)}</p>
                          )}
                          <hr className="my-2" />
